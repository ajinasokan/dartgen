...
Maps with primitive non-string keys are serialized with string JSON keys and parsed back to their Dart key type:
...
Generated `toMap()` output for `addressMap` looks like:
...
When the same JSON is read with `Person.fromMap()` or `Person.fromJson()`, the map keys are restored as `int`.

Generic model fields can also be serialized without forcing every list or map callback to be `dynamic`. DartGenerate keeps concrete model values typed and only falls back to dynamic calls for generic type parameters:
...
This generates serialization shaped like:

```dart
Map<String, dynamic> serialize() => {
      'items': items.map((i) => (i as dynamic)?.serialize()).toList(),
      'values': values.map<String, dynamic>(
          (k, v) => MapEntry(k, (v as dynamic)?.serialize())),
    };
```
